...
@pytest.mark.skipif(not
                    os.path.exists(get_cfg()["paths"]["training_data_path"]),
                    reason="Data files not found")
def test_data_shape():
    cfg = get_cfg()
    data = load_data(cfg["paths"]["training_data_path"])
    assert len(data) == 66595, \
        "Dataset did not have the correct number of samples"
    assert data.shape[1] == cfg["hyperparameters"]["input_size"] + 1, \
...
def test_x_y_split():
    ...
    assert x.shape[1] == cfg["hyperparameters"]["input_size"], \
        "Features do not have the correct shape"
    assert len(y.shape) == cfg["hyperparameters"]["output_size"], \
...
def test_normalization():
    ...
    assert all((pd.isna(x_val) or (x_val >= 0 and x_val <= 1))
               for x_val in x_vals), \
