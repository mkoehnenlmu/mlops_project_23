...
We managed dependencies in our project using a combination of tools and approaches to ensure a streamlined environment setup process for new team members. First, we utilized pip for Python package management. The list of project-specific Python dependencies was stored in different requirements.txt file, making it easy to maintain and reproduce the environment needed for development, training or also only inference. We install different dependencies in the respective docker containers.
...
1. **Data (Used)**: The "data" folder played a crucial role in our project. It contained datasets, pre-processing scripts, and data-related utilities. This folder was essential for data ingestion and preparation.
...
3. **Models (Used)**: The "models" folder stored our trained machine learning models, model checkpoints, and serialization files. It was crucial for model storage and versioning.
...
10. **.dvc (Used)**: We also included a ".dvc" folder to integrate Data Version Control (DVC) into our project. DVC helped us manage large datasets efficiently.
...
In our project, we emphasized code quality by implementing multiple unit tests, type hints, and comprehensive docstrings. The tests are run on every push to a pull request aiming to be merged into the main branch.
...
1. **Branch Development**: We (mostly) followed a branch-per-feature approach, where each new feature or bug fix was developed on its dedicated branch. This isolated changes and minimized conflicts during development, though we did go through some merge conflicts along the way, mostly because we were still setting things up during development.
...
3. **Code Review**: PRs underwent a code review process by team members. This review step helped identify and rectify potential issues.
...
1. **Tests**: We run unit tests to validate the correctness of our code. These tests cover various aspects of our application to ensure its reliability, as mentioned before.
...
4. **Dependency Management**: We maintain a comprehensive list of dependencies and their versions. This ensures that the software environment remains consistent and reproducible across different runs and environments.
...
1. **Containerization**: We encapsulated our model training and its associated environment within a Docker container. This container included Python dependencies, libraries, and specific configurations, ensuring that the model's behavior remained consistent across different environments.
...
3. **Deployment**: We use Docker to deploy our trained models for inference on Google Compute Engine. This provided a seamless and consistent environment for model serving.

To run our Docker images locally, we typically used the following command:
...
```bash
docker run -it -p 8080:8080 training:latest
```
...
1. **Local Development**: Whenever possible, we conducted local development. This environment provided access to standard debugging tools, allowing us to set breakpoints, inspect variables, and step through code more effectively.
...
1. **Google Cloud Compute Engine**: We deployed virtual machines on Compute Engine for both inference and training tasks. These VMs provided scalable computing resources for our machine learning workloads.

2. **Google Cloud Storage**: We utilized Google Cloud Storage buckets for storing various project artifacts, including datasets, trained models, and experiment results. This offered secure, scalable and version controlled object storage for our data and assets.
...
Both types of VMs were hosted in the `europe-west3-c` zone and were associated with specific service accounts, granting them necessary permissions for cloud platform access. Compute Engine ensured our project had the computing power required for training and serving machine learning models efficiently in a scalable and cloud-native manner.
